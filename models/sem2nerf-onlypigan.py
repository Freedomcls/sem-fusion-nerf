...
class Sem2NeRF(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.latent_code = frequencies, phase_shifts
    ...
    def set_devices(self, gpu_ids, mode='train'):
        ...
        # self.z = self.z.to(self.enc_device)
        ...
        if hasattr(self, 'latent_code'):
            self.latent_code = [x.to(self.dec_device) for x in self.latent_code]

        if self.opts.distributed_train and mode == 'train':
            # self.encoder = torch.nn.parallel.DistributedDataParallel(self.encoder, device_ids=[self.enc_device], broadcast_buffers=False, find_unused_parameters=True)
            ...
            # self.decoder = torch.nn.parallel.DistributedDataParallel(self.decoder, device_ids=[self.dec_device], broadcast_buffers=False, find_unused_parameters=True)
    ...
    def forward(self, pose, return_latents=False, iter_num=0):
        ...
        # codes = self.latent_code
        # print(self.z.requires_grad,'z')
        cur_pigan_env['camera_pose'] = pose[:batch_size].to(self.dec_device)
        # print('curpigan_pose',pose.to(self.dec_device))
        ...
        dec_out_dict = self.decoder(**cur_pigan_env)

        # gather results
        out_dict = {'images': dec_out_dict['pixels']}
        # if return_latents:
        #     out_dict['latents'] = (codes, (self.latent_code[0].repeat(codes[0].shape[0], 1),
        #                                 self.latent_code[1].repeat(codes[1].shape[0], 1)))
        if self.opts.patch_train:
            out_dict['sample_pattern'] = dec_out_dict['sample_pattern']

        return out_dict

    def forward_inference(self, pose, latent_mask=None,
        ...
        if pose is not None:
            ...
            cur_pigan_env['camera_pose'] = pose.to(self.dec_device)

        # decode latent code to output
        images, _ = self.subnet_attri_getter('decoder').staged_forward(**cur_pigan_env)
    ...
    def requires_grad(self, dec_flag=False):
