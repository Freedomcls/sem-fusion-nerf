...
class ImplicitGenerator3d(nn.Module):
    def __init__(self, siren, z_dim, **kwargs):
        ...
        self.z = torch.randn((1,256),requires_grad=True)
        self.z = nn.Parameter(self.z)

        self.z_val = torch.randn((1,256), device='cpu')
        self.z_val = nn.Parameter(self.z_val)
        ...
        # self.latent_code = [x.to(self.siren.device) for x in latent_code]
    ...
    def forward(self, img_size, fov, ray_start, ray_end, num_steps, h_stddev, v_stddev, camera_pose, hierarchical_sample, opts, sample_dist=None, lock_view_dependence=False, **kwargs):
    # def forward(self, img_size, fov, ray_start, ray_end, num_steps, h_stddev, v_stddev, camera_pose, hierarchical_sample, opts, sample_dist=None, lock_view_dependence=False, **kwargs):
        """
        ...
        z = self.z
        # frequencies, phase_shifts = self.latent_code
        batch_size = z.shape[0]
        # print(frequencies.requires_grad,'frequencies')
       ...
       # Generate initial camera rays and sample points.
        with torch.no_grad(): 
            ...
            transformed_points, z_vals, transformed_ray_directions, transformed_ray_origins = transform_sampled_points(
                points_cam, z_vals, rays_d_cam, h_stddev=h_stddev, v_stddev=v_stddev, camera_pose=camera_pose, device=self.device, mode=sample_dist)
    ...
    def generate_avg_frequencies(self):
        ...
        z = torch.randn((1, self.z_dim), device=self.siren.device)
        z = nn.Parameter(z)
        with torch.no_grad():
            frequencies, phase_shifts = self.siren.mapping_network(z)
    ...
    # def staged_forward(self, img_size, fov, ray_start, ray_end, num_steps, h_stddev, v_stddev, camera_pose, psi=1, lock_view_dependence=False, max_batch_size=50000, depth_map=False, near_clip=0, far_clip=2, sample_dist=None, hierarchical_sample=False, **kwargs):        
    #     """
    #     Similar to forward but used for inference.
    #     Calls the model sequencially using max_batch_size to limit memory usage.
    #     """

    #     z = torch.randn((1,256), device=self.siren.device)
    #     z = nn.Parameter(z)
    #     # frequencies, phase_shifts = self.latent_code
    #     batch_size = z.shape[0]
    ...
    #         transformed_points, z_vals, transformed_ray_directions, transformed_ray_origins= transform_sampled_points(points_cam, z_vals, rays_d_cam, h_stddev=h_stddev, v_stddev=v_stddev, camera_pose=camera_pose, device=self.device, mode=sample_dist)
