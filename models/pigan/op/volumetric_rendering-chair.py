...
def get_initial_rays_trig(n, num_steps, device, fov, resolution, ray_start, ray_end, opts=None):
    ...
    if opts is not None and opts.patch_train:
        ...
        sample_pattern = get_sampling_pattern(n, device, opts.resolution_vol, opts.ray_scale_anneal, 
                                                opts.ray_min_scale, opts.ray_max_scale,
                                                opts.ray_random_scale, opts.ray_random_shift, opts.iter)
...
def transform_sampled_points(points, z_vals, ray_directions, device, h_stddev=1, v_stddev=1, camera_pose=[], mode='normal'):
    ...
    cam2world_matrix = camera_pose
    # cam2world_matrix = torch.tensor(cam2world_matrix, dtype=torch.float, device=device)
    # cam2world_matrix = cam2world_matrix.astype(np.float32)   
    # print('camerpose',cam2world_matrix)
    ...
    # print(transformed_ray_directions)
    ...
    return transformed_points[..., :3], z_vals, transformed_ray_directions, transformed_ray_origins
...
def get_sampling_pattern(batch_size, device, resolution_vol, ray_scale_anneal, ray_min_scale, ray_max_scale, 
                        ray_random_scale, ray_random_shift, iter):
    w, h = torch.meshgrid([torch.linspace(-1, 1, resolution_vol),
                            torch.linspace(-1, 1, resolution_vol)])
    ...
    selected_index = torch.stack([h, w], dim=-1).to(device)
