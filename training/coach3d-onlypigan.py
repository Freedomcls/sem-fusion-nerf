...
class Coach3D:
    ...
    def train(self):
        ...
        self.net.train()
        self.net.requires_grad(dec_flag=self.opts.train_decoder)  # set grad accordingly
        if self.opts.dis_lambda > 0:
            self.net_dis.train()
        ...
        while self.global_step < self.opts.max_steps:
            for batch in self.train_dataloader:
                ...
                if self.opts.dis_lambda > 0:
                    self.net.requires_grad(dec_flag=self.opts.train_decoder)
                ...
                # y, pose = batch['to_im'], batch['im_pose']
                ...
                out_dict = self.net(pose, return_latents=False, iter_num=self.global_step)
                ...
                loss, loss_dict = self.calc_loss(y, y_hat, train_rand_pose=train_rand_pose)
                # loss, loss_dict = self.calc_loss(x, y, y_hat, train_rand_pose=train_rand_pose)

                # with torch.autograd.set_detect_anomaly(True):
                # print(y_hat.requires_grad,'y_hat')
                loss.backward()
                self.optimizer.step()

                # for name, param in self.net.decoder.named_parameters():
                #     if param.grad is not None:
                #         print(name, param.grad.reshape(-1)[:10])
                #     else:
                #         print(name,None)
                #     print(name, param.reshape(-1)[:10])
                    # break
    ...
    def validate(self, log_info=True):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            with torch.no_grad():
                x = x.float()
                out_dict = self.net.forward_inference(pose, latent_mask=self.val_latent_mask,
                ...
                _, cur_loss_dict = self.calc_loss(y, y_hat, phase='val')
                # _, cur_loss_dict = self.calc_loss(x, y, y_hat, phase='val')
                # # add two novel views
    ...
    def calc_loss(self, y, y_hat, phase='train', train_rand_pose=False):
    ...
    def parse_and_log_test_images(self, x, y, y_hat, title, subscript=None, display_count=2):
